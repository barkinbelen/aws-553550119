...
resource "aws_security_group" "web_server_sg" {
  ...
  ingress {
    ...
    to_port         = 80
    protocol        = "tcp"
    security_groups = var.create_load_balancer ? [aws_security_group.web_server_alb_sg[0].id] : []
    cidr_blocks     = var.create_load_balancer ? [] : ["0.0.0.0/0"]
  }
...
resource "aws_lb" "web_server_alb" {
  ...
  security_groups    = [aws_security_group.web_server_alb_sg[0].id]
...
resource "aws_lb_listener" "web_server_listener" {
  count             = var.create_load_balancer ? 1 : 0
  load_balancer_arn = aws_lb.web_server_alb[0].arn
  port              = "80"
  protocol          = "HTTP"

  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.web_server_tg[0].arn
  }
...
resource "aws_autoscaling_group" "web_server_asg" {
  ...
  vpc_zone_identifier       = var.create_load_balancer ? var.private_subnet_ids : var.public_subnet_ids
  target_group_arns         = var.create_load_balancer ? [aws_lb_target_group.web_server_tg[0].arn] : []
  min_size                  = 1
  max_size                  = 4
  desired_capacity          = 1
